...
public class CalcState extends AbstractStatus {
    ...
    public Status handle() {
      try{  BigDecimal pricePerKg = new PriceSelector().selectPriceValue(pricePerKgMap, userData.get(0));
        BigDecimal pricePerKm = new PriceSelector().selectPriceValue(pricePerKmMap, userData.get(1));
        BigDecimal result = new CalcImpl().handle(userData.get(0), pricePerKg, userData.get(1), pricePerKm);
